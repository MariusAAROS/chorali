...
import framework, concept_mapper, ordering, berkeleyparser, compression, util, prob_util
from globals import *
import ilp, text
...
def parse_options():
    ...
    parser.add_option('--splitta-model', dest='splitta_model', default=None, type='str',
                      help='model directory for sentence splitter')
    parser.add_option('-i', '--is_clean', dest='is_clean', default=False,
                      action='store_true',
                      help='If True, input files are raw text and xml otherwise')
...
def run_standard(options, max_sents=10000):
    ...
    if options.compress:
        for problem in task.problems:
            ...
            if options.task in ['u09', 'u08', 'chorali']: output_id = problem.id[:5]+problem.id[6:]
...
def dump_data(path):
    ...
    #os.mkdir(path)
    #uncomment next line to load precomputed clusters
    #os.system('cp /scratch_global/gael/maali/2004/clusters/* %s' %path)
    import sentence_cleaner
...
if __name__ == '__main__':
    ...
    if options.task in ['chorali']: framework.setup_Chorali(task)
    elif options.task in ['u09', 'u08']: framework.setup_TAC08(task)
    else: framework.setup_DUC_basic(task)
    ...
    framework.setup_DUC_sentences(task, parser, reload=options.reload,
                                  options=options, is_clean=options.is_clean)
