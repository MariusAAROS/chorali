...
def load_sents(path, id):
    ...
    while True:
        if parse_fh:
            [doc, orig, tok, parse, par] = map(str.strip, [doc_fh.readline(), orig_fh.readline().encode("ascii","ignore"),
                                                           tok_fh.readline().encode("ascii", "ignore"), parse_fh.readline(), par_fh.readline()])
        else:
...
def make_concepts(id, path, sents, query):
    ...
    final_concepts_fusion={}
    for i,sent in enumerate(sents):
        sent.concepts = sent.concepts.intersection(final_concept_set)
    for i,sent in enumerate(sents):
        for j,sent2 in enumerate(sents):
            if j > i:
                for concp1 in sent.concepts.copy() :
                    for concp2 in sent2.concepts.copy():
                        if util.same(concp1,concp2):
                                print("FUSE BIGRAMS %s and %s !!!" %(concp1,concp2))
                                sent2.concepts.remove(concp2)
                                sent2.concepts.add(concp1)
                                final_concepts_fusion[concp1]=final_concepts[concp1]+final_concepts[concp2]
                        else:
                                final_concepts_fusion[concp1]=final_concepts[concp1]
				print("DO NOT FUSE!!")
    final_concepts=final_concepts_fusion
