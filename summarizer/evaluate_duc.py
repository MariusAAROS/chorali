...
def create_config(model_dir, peer_dir, chop_annotator_id=False):
...
def run_rouge(executable, config_file, length, verbose=False):
    ...
    for metric in metrics:
        vals = [score for topic, score in scores[metric].items() if '-A' in topic]
        try: means[metric] = sum(vals) / len(vals)
        except: means[metric] = 0
    print('AVG (A)  %s' %' '.join(['%1.4f  ' %means[metric] for metric in metrics]))

    means = {}
    for metric in metrics:
        vals = [score for topic, score in scores[metric].items() if '-B' in topic]
        try: means[metric] = sum(vals) / len(vals)
        except: means[metric] = 0
    print('AVG (B)  %s' %' '.join(['%1.4f  ' %means[metric] for metric in metrics]))
    ...
    print('AVG      %s' %' '.join(['%1.4f  ' %means[metric] for metric in metrics]))


if __name__ == '__main__':
    if len(sys.argv) < 5 or len(sys.argv) > 6:
        sys.stderr.write('USAGE: %s <path_to_rouge_script> <model_dir> <peer_dir> <length> [chop_annotator_id]\n' %sys.argv[0])
        sys.exit(1)
    if len(sys.argv) == 6:
        config_file = create_config(sys.argv[2], sys.argv[3], bool(sys.argv[5]))
    else:
        config_file = create_config(sys.argv[2], sys.argv[3])
    run_rouge(sys.argv[1], config_file, length=int(sys.argv[4]), verbose=False)
