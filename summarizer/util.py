...
def is_punct(text):
    ...
    for letter in text.lower():
        if letter in set('abcdefghijklmnopqrstuvwxyz1234567890'):
            return False
    return True


stopwords = set(open(
    os.path.join(os.environ['ICSISUMM'],
                 'data/stopwords.english')).read().splitlines())
...
def get_ngrams(sent, n=2, bounds=False, as_string=False):
    ...
    elif type(sent) == type(u''): words = str(sent).split()
    else:
        print(sent)
        print(type(sent))
...
#    return ' '.join(punkt_word_tokenizer.tokenize(text))
    return ' '.join(nltk.tokenize.word_tokenize(text))


def same(concept1,concept2):
    clist1=concept1.split("_")
    clist2=concept2.split("_")
